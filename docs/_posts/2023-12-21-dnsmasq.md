...
Sirvo dos dominios localmente, `.parchis.org` y ahora `.home.arpa` ([RFC 8375](https://www.rfc-editor.org/rfc/rfc8375.html), al que voy a ir migrando poco a poco. Mis servidores importantes son:
...
Opto por instalar `dnsmasq.d` en el router `cortafuegix`, ofrece ambos DHCP y DNS, es sencillo, rápido y fácil de mantener. Resolverá ambos dominios (`parchis.org, home.arp`), y el resto de consultas las redirigirá al PiHole para que haga lo que sabe hacer muy bien, el sumidero de la publicidad, y si este se ha caído por cualquier motivo, continuará resolviendo upstream en orden estricto.
