...
Dependiendo de cual de las tres elijas conseguirás que X11 en `root` funcione (OK) o falle (FALLA). 
...
Elije la que más te convenga. Aunque no es demasiado purista **en mi caso estoy usando las opciones (1) y (3) simultáneamente** 🤗 y además añado la siguiente línea al final de mi `.bashrc` del usuario `luis` para evitar el SEMIFALLO de "sudo \<comando>": 
