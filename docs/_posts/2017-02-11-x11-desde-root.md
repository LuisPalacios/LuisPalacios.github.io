...
Sin embargo **siempre hay un truco 🤗**, aunque me adelanto un poco, la solución pasa por reutilizar las credenciales X11 del usuario original con el que te conectaste, su `lista de cookies`, que se las prestaremos a `root` usando diferentes métodos, lo vemos más adelante. 
...
Existe la posibildiad de activar el módulo `pam_xauth.so` en la última línea del fichero `/etc/pam.d/su`. Añado lo siguiente
...
Elije la que más te convenga. **En mi caso estoy usando las opciones (1) y (3) simultáneamente** 🤗 y además añado la siguiente línea al final de mi `.bashrc` del usuario `luis` para evitar el SEMIFALLO de "sudo \<comando>": 
