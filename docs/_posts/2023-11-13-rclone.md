...
En este apunte explico cómo gestiono mis datos en mi Mac mini junto con `iCloud` y `rclone` para conseguir la máxima velocidad, acceder desde cualquier lugar a lo principal, contar con un almacén y que sea seguro (ya que "todo puede fallar").

Cuanto con el disco interno del Mac, varios externos, iCloud, un linux remoto con un disco para backups y además estoy pensando en contratar con dropbox o Google. Muchos sitios, diferentes tecnologías, velocidades, necesidades. Me he dado cuenta que `rclone` es la herramienta perfecta.
...
Mis datos están en dos sitios `priv`y `Baul`. La carpeta `priv` está alojada en `iCloud`, es donde tengo los datos principales (accesibles desde el entorno Apple, iPhone, iPad, etc) y está completamente sincronizada en mi Mac. El sitio `Baul` es un disco duro externo SSD (Thunderbolt 4) muy rápido, donde tengo el almacén al que solo necesito acceder cuando trabajo en el Mac; me recuerda al típico disco `D:\` de otra época.

En cuanto a Backups: por un lado uso `TimeMachine` para tener un backup completo del disco principal del Mac. Además un disco externo llamado **Trastero**, un **linux remoto** y en el futuro **Dropbox o Google** para hacer backup de lo importante (`priv` y `Baul`).
...
El disco duro principal interno del Mac "Macintosh HD" (`HD` a partir de ahora) tiene el Sistema Operativo, las aplicaciones y el HOME de mi usuario (`/Users/luis`). Este es el plan de copias de seguridad: 
