...
En este apunte explico cómo trabajo con mis datos en mi Mac mini junto con `iCloud` y `rclone` para ir a máxima velocidad, acceder desde cualquier lugar a los datos principales, conseguir un almacen para los menos accedidos y como "todo puede fallar", contar con al menos 3 copias de seguridad. 

Tengo el disco interno, varios externos, iCloud y un linux remoto con un disco para backups. Además pensando en  contratar con dropbox o Google. 
...
Utilizo un Mac Mini como equipo principal y varios dispositivos secundarios (móvil, iPad, laptop linux). Tengo un directorio principal al que quiero poder acceder desde cualquiera de los dispositivos y un almacén de datos al que solo necesito acceder desde el Mac. 

He decidido poner mis datos en `iCloud` (directorio principal) y un disco externo `Baul` (almacén de datos). Los backups irán a `TimeMachine` y a otro disco externo `Trastero` y un servidor remoto vía VPN llamado `luna` que tiene un disco USB. 

El disco duro principal interno del Mac (Macintosh HD) que llamo `HD` tiene el Sistema Operativo, las aplicaciones y el HOME de mi usuario (`/Users/luis`). Mis datos principales están en iCloud en el directorio `priv`. Ejecuté "Descargar ahora" (desde el Finder) en mi Mac Mini para conseguir una copia completa en local y a la vez sincronizada permanentemente con iCloud. 

Consigo tres beneficios: Tengo dos copias del dato. La copia local está descargada por completo y sincronizada en un disco SSD interno que es con el que trabajo habitualmente y por último, puedo acceder a esta carpeta desde cualquier dispositivo Apple (iPhone, iPad, MBP) con mi usuario iCloud. Más adelante sincronizaré con Dropbox o Google dicha carpeta usando `rclone` para poder acceder desde Linux.
...
| `priv` | iCloud (Datos principales) | Sincronizas en HD. En `Trastero` y linux remoto; ambos vía `rclone`. Más adelante también en Dropbox/Google vía `rclone   ` |
| `Baul` | Disco externo (Datos almacén) | En `Trastero` y en linux remoto; ambos vía `rclone` |
| `Trastero` | Disco externo (solo Backups) | N/A |
...
### Configuración de rclone en Mac Mini
...
Para el típico backup entre dos discos. En este ejemplo quiero sincronizar los directorios `priv` y `work` desde el disco externo `Baul` a `Trastero`. Recomiendo usar `--dry-run` o `check` para probar primero. 

Una forma de hacerlo es ejecutandolo por cada directorio: 

```shell
rclone sync --copy-links Baul:priv Trastero:priv
rclone sync --copy-links Baul:work Trastero:work
```

Otra es usar un fichero de filtraje: `filter-from-Baul.txt` donde especifico qué quiero excluír e incluir:
