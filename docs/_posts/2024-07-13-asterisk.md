...
Si tu operador es Movistar y usas el router "oficial", todo lo que describo aquí entiendo que debería funcionar, porque dicho router debería enrutar los paquetes hacia el "Proxy Movistar" correctamente. Digo debería porque NO lo he probado, pero por lo que he leído por ahí a mucha gente le está fucnionando.

Si tu operador es Movistar y usas otro router tuyo o personalizado, averigua cómo debes configurarlo para hacer routing hacia la IP del proxy.

En el caso de otros operadores y router "oficial", averigua si soportan SIP y sobre todo cómo llegar a la IP de su proxy, estando detrás del router que te hayan puesto. Y lo mismo si tienes otro operador y un router proio, averigua cómo debes hacer routing hacia la IP del proxy.

Dicho esto, comparto mi config. Recordar que el caso de uso de esta configuración es un router linux que accede directamente a la ONT, por lo tanto tiene acceso directo a la vlan6 para datos, vlan2 para IPTV y vlan3 para Voz.

La configuración concreta para la VLAN3 en Ubuntu es la siguiente:

* `/etc/netplan/50-mi-netplan.yaml`. En mi caso este equipo está en una Virtual Machine corriendo en un Proxmox. Muestro parte de la config.
...
***Configuración IP***: Tras el arranque de sistema (o de netplan), así es como quedan mis rutas instaladas, recibo una IP vía DHCP e instalo la ruta estática al Proxy SIP de Movistar.
...
- Poner tu número de teléfono de casa, en varios sitios en ambos ficheros verás el número ficticio `911234567`. Cámbialo al tuyo en todos los sitios donde lo veas.
