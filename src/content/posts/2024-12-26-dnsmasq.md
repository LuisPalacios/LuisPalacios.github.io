...
<img src="/img/posts/logo-dnsmasq.svg" alt="Logo dnsmasq" width="150px" style="float:left; padding-right:25px"  />

En este apunte describo cómo he evolucionado mi servidor DHCP y DNS casero. Hasta ahora tenía un [PiHole]({{< relref "2021-06-20-pihole-casero.md" >}}) dedicado en la red a DHCP, DNS y Sumidero de la publicidad. He decidido migrar a una configuración distinta, **mover ambos servicios DNS y DHCP al router** (linux) de casa.
...
<div class="image-box">
  <img src="/img/posts/2024-12-26-dnsmasq-01.svg" alt="Nueva arquitectura" width="500px" />
  <div class="image-caption">Nueva arquitectura</div>
</div>
...
<div class="image-box">
  <img src="/img/posts/2024-12-26-dnsmasq-04.png" alt="El problema de este setup" width="700px" />
  <div class="image-caption">El problema de este setup</div>
</div>

En ese caso hay que irse al router y usar comandos CLI. No mola. Este es el aspecto que tengo en mi PiHole. Dejo este apunte como referencia, pero me he puesto ya a trabajar en una nueva versión: [Router con PiHole]({{< relref "2025-03-08-router-pihole.md" >}})
...
<div class="image-box">
  <img src="/img/posts/2024-12-26-dnsmasq-02.png" alt="Estructura archivos de configuración" width="800px" />
  <div class="image-caption">Estructura archivos de configuración</div>
</div>
...
### Resolución desde los clientes

Cualquier cliente de la red debería resolver hosts **con o sin** el sufijo `parchis.org`.
Ejemplo: tanto `nslookup luix-wifi` como `nslookup luix-wifi.parchis.org` deben devolver la misma IP.

Si el cliente no añade el sufijo automáticamente, la resolución falla. La responsabilidad de expandir `luix-wifi` a `luix-wifi.parchis.org` **es siempre del propio cliente**.

Opciones:

- **DHCP**: el servidor Pi-hole debe anunciar el *DNS search domain*. En mi configuración ya lo hace.
- **IP estática**: el cliente debe configurarlo manualmente.

**Recomendación**: usar siempre DHCP con reservas por MAC para nodos fijos. Si necesitas estáticos:

- **macOS**: Preferencias de Red → DNS → añadir `parchis.org` en *Search Domains*.
- **Linux**: en `/etc/resolv.conf` o configuración de `systemd-resolved`, añadir: `search parchis.org`
- **Windows 11**:
  - GUI: `ncpa.cpl` → adaptador → Propiedades → IPv4 → Avanzado → pestaña DNS → definir *Sufijo DNS de esta conexión* = `parchis.org`.
...
Compruebo que la resolución DNS e inversa son correctas. Como puedes observar no necesito añadir el nombre del dominio, que se autocompleta solo Lo he comprobado con clientes Windows, Mac, Linux y dispositivos móviles.
...
<div class="image-box">
  <img src="/img/posts/2024-12-26-dnsmasq-03.png" alt="Flujo de resolución DNS" width="800px" />
  <div class="image-caption">Flujo de resolución DNS</div>
</div>
