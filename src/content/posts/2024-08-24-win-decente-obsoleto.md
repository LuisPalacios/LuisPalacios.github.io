---
title: "Un Windows decente (obsoleto)"
date: "2024-08-24"
...
<img src="/img/posts/logo-windows.svg" alt="logo linux desarrollo" width="150px" height="150px" style="float:left; padding-right:25px"  />

En este apunte explico cómo parametrizo un Windows 11 que voy a usar para desarrollo de software, pruebas o demos. No necesito florituras ya que no va a tener datos sensibles, quiero su esencia, a pelo, con pocas aplicaciones, algo de navegación y punto. Al final se ha convertido en un ejercicio técnico, quitarle todo lo que pueda, anuncios, Edge, extras, instalaré drivers mínimos, una cuenta local, que arranque y esté disponible lo antes posible.

Aviso: un año después decidí hacer un nuevo [Windows 11 decente]({{< relref 2025-08-03-win-decente.md >}}) (2025).
...
- Configurar [un Windows 11 decente]({{< relref 2025-08-03-win-decente.md >}}) quitando la morralla (Versión nueva).
...
**¿Qué es el debloat o eliminación del bloatware?**. Se refiere a las aplicaciones y servicios preinstalados en un sistema operativo que, aunque no esenciales, consumen recursos y pueden afectar al rendimiento del equipo.
...
Como decía, todavía no he usado ninguno, pero el día que lo haga me los estudiaré a fondo, incluidos los fuentes, seguiré las instrucciones y haré backup antes de ejecutar, algunas modificaciones pueden ser irreversibles y afectar la estabilidad del equipo.

## Primeros pasos

A partir de aqui me centro en mi proceso manual. Tiene menos riesgo porque vas viendo lo que hago, pero es mucho más largo. Parto de un windows 11 desde cero. Puede que sirva para uno con el que llevas tiempo trabajando, pero no lo he probado. En mi caso lo hice después de instalar [Dualboot Linux Windows]({{< relref "2024-08-23-dual-linux-win.md" >}}) o [VMWare en Windows]({{< relref "2024-08-26-win-vmware.md" >}}), es decir recién instalado en bare metal o en una VM.

Lo ***básico*** antes de meterle mano al bloatware.

- Me instalo uno de estos navegadores: [Vivaldi](https://vivaldi.com/es/), [Chrome](https://www.google.com/intl/es_es/chrome), [Brave](https://brave.com/es/) y [Firefox](https://www.mozilla.org/es-ES/firefox/new/), [Opera](https://www.opera.com/es).
...
| Nota: ¿Porqué no uso Edge?. Se que Edge utiliza WebView2, que está basado en Chromium (poyecto open source), que usa Blink como motor de renderizado y V8 como motor de JavaScript. Hasta ahí bien, parece la mejor opción para Windows, pero ahí está su problema, es mono-plataforma, muy endogámico. En mi caso quiero una plataforma única de navegación con soporte de perfiles, super compatible y multiplataforma (Windows, Linux, MacOS). Ahí es donde entran esos proyectos que menciono, que además todos (menos firefox) usan Chromium. De momento estoy con Google Chrome, pero echando un ojo a Vivaldi, que es sinceramente un trabajo impresionante. |
...
- Durante la [instalación dualboot]({{< relref "2024-08-23-dual-linux-win.md" >}}) usé Ratón/Teclado USB's por cable y al terminar añadí un Ratón Logitech (con un plugin USB) y el teclado Logitech K380 vía bluetooth
...
- Opción 2: [Microsoft Activation Script (MAS)](https://github.com/massgravel/Microsoft-Activation-Scripts), se trata de una activador de código abierto para Windows y Office que incluye los métodos de activación HWID, Ohook, TSforge, KMS38 y Online KMS.
  - Siguiendo su documentación, así se licencia un [VM Windows 11 Pro]({{< relref "2024-08-26-win-vmware.md" >}}):
...
<div class="image-box">
  <img src="/img/posts/2024-08-24-win-decente-01.png" alt="Versión minimalista de Buscar" width="400px" />
  <div class="image-caption">Versión minimalista de Buscar</div>
</div>
...
Un caso de uso interesante es remapear el teclado. Tengo un apunte donde describo cómo uso [Barrier para imitar la funcionalidad KVM]({{< relref "2024-06-13-kvm.md#unificar-las-teclas-de-control" >}}), al final del mismo trato el tema de remapeo.

***Visual Studio Code***: Tampoco requiere mucha presentación, un editor de código fuente que incluye un terminal integrado. Puedo abrir diferentes **terminales integrados** dentro de la misma ventana de VS Code, como CMD, PowerShell, [Git Bash](https://git-scm.com/install/windows) o WSL.
...
Al seguir los pasos anteriores obtengo un Windows 11 mucho más limpio, rápido y libre de distracciones. Además de las optimizaciones mencionadas, dejo aquí algunas cosillas adicionales, que pueden llevarte un paso más allá, ten en cuenta que a partir de aquí ya es decisión personal.
...
<div class="image-box">
  <img src="/img/posts/2024-08-24-win-decente-03.jpg" alt="Versión minimalista del escritorio" width="800px" />
  <div class="image-caption">Versión minimalista del escritorio</div>
</div>
...
- `sfc`. Analizar la integridad de todos los archivos de sistema y solucionar problemas en los mismos. ***AVISO!!***: Microsoft tiene un problema conocido que lleva años sin resolverse. De hecho a mi me ha pasado. Al ejecutarlo por primera vez: `sfc /SCANNOW` encuentra un problema en el archivo `bthmodem.sys` y lo elimina.
  - Indica que ha encontrado corrupción y si miras su log `\Windows\Logs\CBS\CBS.log` verás que se refiere a `Corrupt File: bthmodem.sys`.
  - Ejecuta `dism /online /cleanup-image /restorehealth` para resolver el entuerto.
