...
Ese fichero es el del contenedor LXC `nut` (ojo que en tu caso puedes haber asignado otro número). En él tenemos que añadir lo siguiente al final. Es importante montar el directorio `/dev/bus/usb` completo con `create=dir` (en lugar de montar el archivo de un dispositivo concreto) para que, si el SAI cambia de número de puerto USB tras un reinicio, el contenedor lo siga detectando dinámicamente sin romper la conexión:
