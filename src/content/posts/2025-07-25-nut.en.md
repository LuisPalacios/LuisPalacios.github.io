...
This file belongs to the LXC container `nut` (note that in your case you may have assigned a different number). We need to add the following at the end. It is important to mount the whole `/dev/bus/usb` directory with `create=dir` (instead of mounting the file for one specific device) so that, if the UPS ends up on a different USB port number after a reboot, the container keeps detecting it dynamically without breaking the connection:
